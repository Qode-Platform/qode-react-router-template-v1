 ...
 *
 * HOW TO FIX AN OFFENDER (React Router (Vite))
 * --------------------------------------
 * React Router resolves ROUTES under the `basename` set in
 * react-router.config.ts (from BASE_PATH), so `<Link>` / `<NavLink>` /
 * `navigate()` are prefixed for you, and Vite rewrites imported assets
 * (`import logo from './logo.svg'`). Raw string URLs are NOT touched.
 *
 * NOTE: the Vite mechanism for this is normally `import.meta.env.BASE_URL`, but
 * in THIS template Vite's `base` is deliberately left unset (BASE_PATH contains
 * a colon, which crashes path-to-regexp inside react-router-serve - see the
 * comment in vite.config.ts). So `import.meta.env.BASE_URL` is "/" here and is
 * NOT a reliable source. Use the router's basename instead:
 *
